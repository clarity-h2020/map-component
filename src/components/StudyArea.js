	...
	setStudyURL(studyUuid, hostName) {
		...
		fetch(
			...
			.then(function(data) {
				...
				fetch(hostName + '/rest/study/' + data.data[0].attributes.drupal_internal__id + '/area?_format=json', {
					credentials: 'include'
				})
					.then((resp) => resp.json())
					.then(function(data) {
				...
				// FIXME! USE INCLUDES!
				fetch(
					data.data[0].relationships.field_city_region.links.related.href.replace('http:', _this.protocol),
					{ credentials: 'include' }
				)
					.then((resp) => resp.json())
					.then(function(data) {
