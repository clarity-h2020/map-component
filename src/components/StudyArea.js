...
export default class StudyArea extends React.Component {
   ...
   */
	setStudyURL(studyUuid, hostName) {
		...
		fetch(
			hostName +
				'/jsonapi/group/study?filter[id][condition][path]=id&filter[id][condition][operator]=%3D&filter[id][condition][value]=' +
				studyUuid +
				'&' +
				includes,
			{ credentials: 'include' }
			...
			.then(function(data) {
				if (
					data != null &&
					data.data[0] != null &&
					data.data[0].attributes.field_area != null &&
					data.data[0].attributes.field_area.value != null
				) {
					var wkt = new Wkt.Wkt();
				...
				var cityRequired =
					calculationMethodInclude != null &&
					calculationMethodInclude.attributes.field_calculation_method_id.includes('emikat');
				...
				if (
					cityObject != null &&
					cityObject.attributes.field_boundaries != null &&
					cityObject.attributes.field_boundaries.value != null
				) {
					var wkt = new Wkt.Wkt();
